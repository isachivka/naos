...
function createSlateAutolink(token, fields) {
  return getSample(token)
    ...
    .then(flatFields => fields.map(field => ({
      ...field,
...
function getStructure(token) {
  return fetch(api.structure.url, getHeaders(token, api.structure.method))
    .then((response) => response.json())
}

function getSample(token) {
  return fetch(api.sample.url, getHeaders(token, api.sample.method))
    .then((response) => response.json())
}
